...
# wind_dir_icon = plt.imread('./wind-dir-icon.jpg')
# plt.imshow(wind_dir_icon, origin='lower', extent=(5, 5, 5, 5))
# plt.text(x_dates[0], 26 , 'A')
x_ticks = ax.get_xticks(minor=True)

for xd in np.arange(0, len(x_dates), 3):
    icon = wind_dir_icon.rotate(wind_dir[xd])
    if xd < 70:
        plt.figimage(icon, x_ticks[xd], 650)
