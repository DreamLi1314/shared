 ...
 * @description
 */
public class CustomProducerCallbackPartions {
   public static void main(String[] args) {
      ...
      // 2. 发送数据
      for (int i = 0; i < 10; i++) {
         producer.send(new ProducerRecord<>("first", 2, "", "Hello Kafka + " + i), (meta, exp) -> {
            if(exp == null ) {
               System.out.println(meta.topic() + "---" + meta.partition());
            }
         });
//         producer.send(new ProducerRecord<>("first", "tableName", "Hello Kafka + " + i), (meta, exp) -> {
//            if(exp == null ) {
//               System.out.println(meta.topic() + "---" + meta.partition());
//            }
//         });
